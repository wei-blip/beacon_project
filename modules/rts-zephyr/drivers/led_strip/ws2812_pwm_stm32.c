 ...
 */
static inline bool num_pixels_ok(const struct ws2812_pwm_cfg *cfg,
                                 size_t num_pixels) {
    size_t nbytes;
    bool overflow;
...
{                                                                   \
    ...
    .period  = DT_INST_PWMS_PERIOD_BY_IDX(idx, 0),                  \
    .flags   = DT_INST_PWMS_FLAGS_BY_IDX(idx, 0),                   \
}

#define WS2812_NUM_COLORS(idx) (DT_INST_PROP_LEN(idx, color_mapping))
#define WS2812_PWM_NUM_PIXELS(idx) (DT_INST_PROP(idx, chain_length))
#define WS2812_PWM_BUFSZ(idx) \
    (WS2812_NUM_COLORS(idx) * WS2812_PWM_NUM_PIXELS(idx))
...
#define WS2812_PWM_DEVICE(idx)                                              \
    ...
    DEVICE_DT_INST_DEFINE(idx,                                              \
                  ws2812_pwm_init,                                          \
                  NULL,                                                     \
                  &ws2812_pwm##idx##_data,                                  \
                  &ws2812_pwm_##idx##_cfg,                                  \
                  POST_KERNEL,                                              \
